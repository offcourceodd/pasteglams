...
float CMovementSimulation::GetPredictedDelta(CBaseEntity* pEntity)
{
	...
	{
		float flSum = 0.f;
		for (float fl : vSimTimes)
			flSum += fl;
		return std::max(flSum / vSimTimes.size(), static_cast<float>(TICK_INTERVAL));
	}
	case 1:
		return std::max(*std::max_element(vSimTimes.begin(), vSimTimes.end()), static_cast<float>(TICK_INTERVAL));
	}
	return TICK_INTERVAL;
}
