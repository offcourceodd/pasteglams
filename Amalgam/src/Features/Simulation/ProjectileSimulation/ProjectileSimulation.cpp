...
int CAimbotProjectile::CanHit(Target_t& tTarget, CTFPlayer* pLocal, CTFWeaponBase* pWeapon, bool bUpdate)
{
	...
	for (int i = 1 - TIME_TO_TICKS(m_tInfo.m_flLatency); i <= iMaxTime; i++)
	{
		...
		// Snapshot the *predicted* movement state at this ticka
		...
		// Build positions to test at this ticka
			...
			&& i >= iReactionDelay && (int)vSnapshots.size() > iReactionDelay)
		{
			...
			// Counter-strafe keys are pressed relative to where the player is LOOKING,
			// not relative to their velocity.
			float flViewYaw = tTarget.m_pEntity->As<CTFPlayer>()->GetEyeAngles().y;
			if (flViewYaw == 0.f) // fallback if eye angles unavailable
				flViewYaw = I::EngineClient->GetViewAngles().y;

			vTestPositions = BuildCounterStrafePositions(
				tTarget.m_vPos, tReactionState.vOrigin, tReactionState.vVelocity, flViewYaw,
